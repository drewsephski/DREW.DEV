...
export const AceternityGlareCard = ({
  children,
  className,
  contentClassName,
}: {
  ...
  return (
    <div
      ...
      className="relative isolate [contain:layout_style] [perspective:600px] transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] will-change-transform w-full [aspect-ratio:16/14] sm:[aspect-ratio:16/14] xs:[aspect-ratio:16/15] [aspect-ratio:16/16]"
      ref={refElement}
      onPointerMove={event => {
      ...
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-800 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden">
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))]">
          <div className={cn('h-full w-full bg-slate-950', className)}>
            <div className={cn('p-2 xs:p-3 sm:p-4 flex flex-col items-center justify-center text-center overflow-hidden', contentClassName)}>
              {children}
            </div>
          </div>
        </div>
